...
class BloomFilter : public boost::noncopyable {
 public:
  ...
  bool isMatch(std::string_view key) {
    auto idx_1 = XXHash64::hash(key.data(), key.length(), seed_);
    auto idx_2 = std_hash(key);
    auto idx_3 = CityHash64WithSeed(key.data(), key.length(), seed_);
    auto idx_4 = util::Hash64WithSeed(key.data(), key.length(), seed_);
    std::cout << "idx1 = " << idx_1 << " idx2 = " << idx_2 << std::endl;
    std::cout << "idx3 = " << idx_3 << " idx4 = " << idx_4 << std::endl;
    if (filter_.test(idx_1 % N) && filter_.test(idx_2 % N) &&
        filter_.test(idx_3 % N) && filter_.test(idx_4 % N)) {
      return true;
    }
    return false;
  }
 ...
 private:
  uint64_t seed_;
  std::hash<std::string_view> std_hash;
  std::bitset<N> filter_;
};

int main() {
  BloomFilter<10240000> filter;
  filter.insert("123");
  std::cout << filter.isMatch("123") << std::endl;
  filter.insert("123123123");
  std::cout << filter.isMatch("123123123") << std::endl;
  return 0;
}
